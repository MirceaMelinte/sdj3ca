...
public class DataServerController extends UnicastRemoteObject implements IDataServer {
	...
	private static volatile DataServerController instance = null;
	private static Connection connection;
	private List<IObserver> observers = new ArrayList<>();

	private DataServerController() throws RemoteException {
		super();	
	}
	...
	public void begin() {
		try {
			...
			DataServerController.connection = PersistenceConfig.establishConnection(connection);
			System.out.println("Data server is running... ");
			DataServerController.addNonExistentEntities();
	...
	public CarList executeGetAllCars() throws RemoteException, SQLException {
		try {
			PreparedStatement carStatement = DataServerController.connection
	...
	public PartList executeGetAllParts() throws RemoteException, SQLException {
		try {
			PreparedStatement selectStatement = DataServerController.connection
	...
	public PalletList executeGetAllPallets() throws RemoteException, SQLException {
		try {
			PreparedStatement palletStatement = DataServerController.connection
	...
	public PartList executeGetPartsByProduct(int productId) throws RemoteException {
		try {
			PreparedStatement selectStatement = DataServerController.connection.prepareStatement(
	   ...
	   public PartList executeGetPartsByCar(String chassisNumber) throws RemoteException, SQLException
	   {
	      try 
	      {
	         PreparedStatement statement = 
	               DataServerController.connection.prepareStatement("SELECT id, type, weight FROM Part "
	         ...
	         SQLException
	   {
	      ...
	      PreparedStatement partStatement = 
	            DataServerController.connection.prepareStatement("SELECT id, type, weight, productId FROM Part "
	                  + "WHERE carId = (SELECT carId FROM Car WHERE chassisNumber = ?)");
	      
	      PreparedStatement productStatement = 
	            DataServerController.connection.prepareStatement("SELECT id, name, type FROM Product "
         ...
         while (resultSet.next()) {
			...
			if (product.getPartList().count() > 0) {
				product.getPartList().getList().forEach(part -> {
					try {
						PreparedStatement updateStatement = DataServerController.connection
	...
	public Part executeUpdatePartPallet(Part part, Pallet pallet) throws RemoteException, SQLException {
		try {
			...
			PreparedStatement updateWeightStatement = DataServerController.connection
   ...
   private static void addNonExistentEntities() throws SQLException {
      try {
         PreparedStatement insertPalletStatement = DataServerController.connection
         ...
         PreparedStatement insertProductStatement = DataServerController.connection
