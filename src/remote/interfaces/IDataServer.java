...
public interface IDataServer extends Remote {
	...
	Car executeGetCarByChassisNumber(String chassisNumber)
	...
	PartList executeGetPartsByPallet(int palletId) throws RemoteException,
			SQLException;

	PartList executeGetPartsByCar(String chassisNumber) throws RemoteException,
			SQLException;

	ProductList executeGetStolenProducts(String chassisNumber)
			throws RemoteException, SQLException;

	// Car executeGetStolenCar(String chassisNumber) throws RemoteException,
	// SQLException;
	Pallet executeUpdatePalletState(Pallet pallet, String state)
			throws RemoteException, SQLException;

	Car executeUpdateCarState(Car car, String state) throws RemoteException,
			SQLException;

	// REGISTER
	Car executeRegisterCar(Car car) throws RemoteException, SQLException;

	Pallet executeRegisterPallet(Pallet pallets) throws RemoteException,
			SQLException;

	Product executeRegisterProduct(Product product) throws RemoteException,
			SQLException;

	Part executeRegisterNewPart(Part part, String chassisNumber)
			throws RemoteException, SQLException;

	// UPDATE
	Part executeUpdatePartPallet(Part part, Pallet pallet)
			throws RemoteException, SQLException;

	Pallet executeUpdateFinishPallet(Pallet pallet) throws RemoteException,
			SQLException;

	Car executeUpdateFinishCar(Car car) throws RemoteException, SQLException;

	// DELETE
	// Maybe few methods for deleting items from database and making sure all
	// caches are updated
